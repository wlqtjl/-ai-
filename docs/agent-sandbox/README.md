...
>
> 1. **脱敏默认开启 与 合并回生产互斥**（脱敏值写回生产 = 数据污染）→ 首版改为「变更导出」；
...
| 要拍板首版范围 | [09 开放问题 #9](09-open-questions.md)（导出 vs 合并，**分叉点**） |
...
4. **首版不写生产库**（第二轮新增）：审批通过后产出变更集导出物，
   由客户既有变更流程执行（[05 文档 1.2](05-approval-and-permissions.md)，待 09 #9 拍板）；
...
外加一个前置决策：**[09 #9](09-open-questions.md) 首版是「变更导出」还是「直接合并回生产」**——
这是分叉点，需一牧内部拍板，未定则本目录的多份文档无法定稿。
