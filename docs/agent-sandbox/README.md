...
> # ⛔ 重要声明：这里全部是设计稿，一牧还没有这个产品
>
> **一牧目前没有为「Agent 数据沙箱」写过任何一行代码。**
>
> 本目录下的 17 份文档、5 份 schema、5 个 Python 脚本，**全部是外部顾问产出的设计草案**，
> 用途是供内部评审和排期讨论。它们**不代表一牧已经具备其中描述的任何能力**。
>
> | 这里有的 | 这里没有的 |
> | --- | --- |
> | 设计文档、接口定义、状态机、指标口径 | ❌ 任何可运行的产品功能 |
> | 几个用来检查文档自身是否自洽的小脚本 | ❌ 任何数据库沙箱实现 |
> | 工作量估算、风险台账、访谈提纲 | ❌ 任何经过验证的性能数字 |
>
> **对外沟通、商务材料、客户演示中，不得引用本目录内容作为一牧已有能力的证明。**
> 需要对客户介绍时，请用专门写的 [客户版说明](customer-guide.md)，它已明确标注为「征求意见稿」。

## 👉 非技术同事请从这三份开始

| 文档 | 用途 | 读完要多久 |
| --- | --- | --- |
| [**大白话总览**](overview-plain.md) | 整套方案在做什么，不用任何术语 | 10 分钟 |
| [**术语对照表**](GLOSSARY.md) | 「凭据保险库」「行级 diff」「GA」这些词到底什么意思 | 随用随查 |
| [**客户版说明**](customer-guide.md) | 给客户看的，只讲怎么用和安全怎么保证 | 8 分钟 |

**另外**：14 份技术文档现在每份顶部都有一段「一句话说明（大白话）」，
不看正文只看那一段，也能知道这份文档在讲什么。
...
| 想做什么 | 看哪个 |
| --- | --- |
| **第一次读，想快速搞懂在做什么** | [大白话总览](overview-plain.md) |
| **某个词看不懂** | [术语对照表](GLOSSARY.md) |
| **要给客户讲** | [客户版说明](customer-guide.md) |
| **想知道哪些前提没被验证** | [00 假设清单](00-assumptions.md) |
...
| 文档 | 说明 |
| --- | --- |
| [`overview-plain.md`](overview-plain.md) | 大白话总览，不含术语，10 分钟读完 |
| [`GLOSSARY.md`](GLOSSARY.md) | 术语对照表，每个词都配「为什么要关心它」 |
| [`customer-guide.md`](customer-guide.md) | 客户版说明，只讲怎么用与安全保证，不含内部实现；已标注为征求意见稿 |

以及 [00 假设清单](00-assumptions.md)：所有技术文档的公共前提，含「若不成立的后果」与「验证方式」。
