...
| `merging` | 审批通过，合并回生产执行中 | 否 | 是 |
| `merged` | 合并完成，等待回收 | 否 | 是 |
...
creating ──► ready ──► checkpointing ──► ready
                │  └──► rolling_back ──► ready
                │  └──► diffing ──► ready
                └──► pending_review ──► merging ──► merged ──► discarding ──► discarded
                          └──(rejected)──► ready
...
1. **`pending_review` 冻结写入**：提交审批后沙盒转为只读，否则审批的内容与实际合并的内容会不一致
   （这是一个容易被忽略的 TOCTOU 风险）。
2. **`merging` 不可中断**：合并过程中禁止丢弃；失败自动进入 `failed` 并触发生产库回退。
3. **`discarded` / `merged` 不可回退**：丢弃不可逆，必须在 MCP 工具描述里对 Agent 明示。
