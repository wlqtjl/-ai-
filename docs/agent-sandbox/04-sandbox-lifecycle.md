...
**这份文档在管「沙箱从生出来到销毁，中间会经历哪些阶段，以及怎么保证它一定会被清理掉」。**

沙箱最大的运维风险不是技术，是**忘了关**。一个人开十个沙箱去测试，测完就走了，
资源就这么一直占着。几个月下来存储被撑爆，谁也说不清哪些还有用。

所以有两条硬规矩：

1. **申请时必须填「用多久」，不填就不给开**，没有默认值——因为凡是有默认值的，
   大家就都用默认值，等于没限制；
2. **时间一到自动回收**，包括销毁那个临时数据库账号。

还有一个容易踩的坑：如果 Agent 在沙箱里存了几个「存档点」，这些存档点会**拖住底层的备份数据不让删**，
时间长了同样撑爆存储。Delphix 就踩过这个坑。所以回收时得按固定顺序清，不能漏。
